...
async function main() {
    ...
    const superAdmins = await db_1.default.query.UserModel.findMany({
        where: (u, { eq, isNull, and }) => and(eq(u.role, "super admin"), isNull(u.agentSerial)),
    ...
    for (const sa of superAdmins) {
        await db_1.default
            ...
            .set({ agentSerial: 0 })
            .where((0, drizzle_orm_1.eq)(schema_1.UserModel.id, sa.id));
        console.log(`✅ Super admin: ${sa.username} → AGT-000`);
    }
    // ── Step 2: Existing admin + support (agentSerial নেই) → sequential ───────
    const agentUsers = await db_1.default.query.UserModel.findMany({
        where: (u, { inArray, isNull, and }) => and(inArray(u.role, ["admin", "support"]), isNull(u.agentSerial)),
    ...
    if (agentUsers.length === 0) {
        ...
        const maxSerialRow = await db_1.default.query.UserModel.findMany({
        ...
        for (const agent of agentUsers) {
            await db_1.default
                ...
                .where((0, drizzle_orm_1.eq)(schema_1.UserModel.id, agent.id));
            console.log(`✅ ${agent.role}: ${agent.username} → AGT-${String(nextSerial).padStart(3, "0")}`);
            nextSerial++;
        }
    }
