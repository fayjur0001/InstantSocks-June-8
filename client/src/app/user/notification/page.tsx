// PATH: client/src/app/user/notification/page.tsx
"use client";
  ...
  return (
    <div className="bg-black p-4 rounded-2xl">
      <div className="mx-auto">
        <h5 className="text-sm font-bold text-white mb-2">Notification Centre</h5>
        <Tabs defaultValue="all" className="w-full">
          <div className="max-w-fit mb-0">
            <TabsList className="flex gap-2 bg-c-slate-900/50 p-1 rounded-[10px] border border-c-slate-800 h-auto">
              {(["all", "today", "thisWeek", "earlier"] as const).map((val) => (
                <TabsTrigger
                  key={val}
                  value={val}
                ...
                >
                  {val === "all" ? "All" : val === "today" ? "Today" : val === "thisWeek" ? "This Week" : "Earlier"}
                </TabsTrigger>
              ))}
            </TabsList>
          </div>
          <TabsContent value="all" className="!mt-0"><TabPane filter="all" /></TabsContent>
          <TabsContent value="today" className="!mt-0"><TabPane filter="today" /></TabsContent>
          <TabsContent value="thisWeek" className="!mt-0"><TabPane filter="week" /></TabsContent>
          <TabsContent value="earlier" className="!mt-0"><TabPane filter="earlier" /></TabsContent>
        </Tabs>
      </div>
    </div>
  );
