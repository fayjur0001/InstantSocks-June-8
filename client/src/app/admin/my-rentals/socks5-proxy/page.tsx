...
export default function AdminMyRentals() {
  ...
  const fetchRentals = useCallback(async (p: number) => {
    ...
    try {
      ...
      toast.error("Failed to load proxy rentals");
    } finally {
      setLoading(false);
    }
  }, []);
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            <p className="text-sm text-c-slate-400">
              {rentals.length === 0 ? "No proxy rentals yet" : "No results match filters"}
            </p>
          </div>
        ) : (
          <ReusableTable
          ...
          <div className="p-5 flex flex-col h-full">
            ...
            <div className="flex flex-col gap-4 text-sm">
              ...
              <div className="flex flex-col gap-1.5 mt-2 text-[12px]">
                <p><span className="text-c-slate-500 mr-2 uppercase">Proxy IP:</span>{selectedProxy.ip}</p>
                <p><span className="text-c-slate-500 mr-2 uppercase">Proxy Port:</span>{selectedProxy.proxyPort || "-"}</p>
                <p><span className="text-c-slate-500 mr-2 uppercase">Internal Port:</span>{selectedProxy.port}</p>
