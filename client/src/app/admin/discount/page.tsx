...
const TIER_STYLES: Record<BadgeTier, TierConfig["color"] & { icon: string }> = {
  Basic: {
...
const DEFAULT_TIERS: TierConfig[] = [
  { tier: "Basic",   minSpend: 0,     maxSpend: 1499,  discount: 0,  ...{ color: TIER_STYLES.Basic,   icon: TIER_STYLES.Basic.icon } },
  { tier: "Bronze",  minSpend: 1500,  maxSpend: 2499,  discount: 5,  ...{ color: TIER_STYLES.Bronze,  icon: TIER_STYLES.Bronze.icon } },
  { tier: "Silver",  minSpend: 2500,  maxSpend: 3999,  discount: 8,  ...{ color: TIER_STYLES.Silver,  icon: TIER_STYLES.Silver.icon } },
  { tier: "Gold",    minSpend: 4000,  maxSpend: 9999,  discount: 10, ...{ color: TIER_STYLES.Gold,    icon: TIER_STYLES.Gold.icon } },
  { tier: "Diamond", minSpend: 10000, maxSpend: null,  discount: 15, ...{ color: TIER_STYLES.Diamond, icon: TIER_STYLES.Diamond.icon } },
];
...
function TierCard({
  ...
  return (
    <div
      className={`relative rounded-xl border ${s.border} ${s.bg} ${s.glow} p-4 flex flex-col gap-3 backdrop-blur-sm transition-all duration-200`}
    >
      {/* ── Header row: icon+name LEFT | discount badge RIGHT ── */}
      <div className="flex items-center justify-between gap-2">
        ...
        <span
          className={`text-[11px] font-extrabold px-2 py-0.5 rounded-full shrink-0 ${
            cfg.discount === 0
              ? "bg-c-slate-800 text-c-slate-500 border border-c-slate-700"
              : s.badge
          }`}
        >
      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-c-slate-500 font-medium uppercase tracking-wide">
            Spend Range
          </span>
        </div>
        ...
        {isEditing ? (
          <div className="flex items-center gap-1.5">
            <span className={`text-[12px] font-semibold ${s.text} shrink-0`}>
              ${cfg.minSpend.toLocaleString()}
            </span>
            {!isDiamond && (
              <>
                ...
                <Input
                  ...
                  onChange={(e) => onDraftChange("maxSpend", e.target.value)}
                  placeholder="Max $"
                  type="number"
                  min={0}
                  step={1}
          ...
          <span className={`text-[13px] font-semibold ${s.text}`}>
            {cfg.maxSpend === null
              ? `$${cfg.minSpend.toLocaleString()}+`
              : `$${cfg.minSpend.toLocaleString()} – $${cfg.maxSpend.toLocaleString()}`}
          </span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {/* Value row: value/input LEFT | edit/save/cancel RIGHT */}
        <div className="flex items-center justify-between gap-2">
          {isEditing ? (
            <div className="flex items-center gap-1.5">
              <Input
          ...
          {isEditing ? (
            <div className="flex items-center gap-1 shrink-0">
              <button
                onClick={onSave}
                className="flex items-center justify-center w-6 h-6 rounded-md bg-c-emerald-700/30 hover:bg-c-emerald-700/60 border border-c-emerald-600/40 transition-colors"
                title="Save"
              >
              ...
              <button
                onClick={onCancel}
                className="flex items-center justify-center w-6 h-6 rounded-md bg-c-rose-500/10 hover:bg-c-rose-500/25 border border-c-rose-500/20 transition-colors"
                title="Cancel"
              >
            ...
            <button
              onClick={onEditStart}
              className="flex items-center justify-center w-6 h-6 rounded-md bg-white/5 hover:bg-white/10 border border-white/10 transition-colors shrink-0"
              title="Edit tier"
            >
...
const MOCK_USERS: UserBadgeRow[] = [
  { id: "1", userId: "#10021", username: "alex_proxy",   totalTopUp: 12500, totalSpend: 11200, badge: "Diamond" },
  { id: "2", userId: "#10022", username: "jane_doe",     totalTopUp: 4800,  totalSpend: 4200,  badge: "Gold"    },
  { id: "3", userId: "#10023", username: "proxyhunter",  totalTopUp: 3100,  totalSpend: 2800,  badge: "Silver"  },
  { id: "4", userId: "#10024", username: "netrunner99",  totalTopUp: 2000,  totalSpend: 1700,  badge: "Bronze"  },
  { id: "5", userId: "#10025", username: "user_basic",   totalTopUp: 800,   totalSpend: 600,   badge: "Basic"   },
  { id: "6", userId: "#10026", username: "socks_master", totalTopUp: 15000, totalSpend: 13500, badge: "Diamond" },
  { id: "7", userId: "#10027", username: "vpn_king",     totalTopUp: 5200,  totalSpend: 4900,  badge: "Gold"    },
  { id: "8", userId: "#10028", username: "newbie_01",    totalTopUp: 150,   totalSpend: 100,   badge: "Basic"   },
];
...
export default function AdminDiscountPage() {
  ...
  const handleEditStart = useCallback((cfg: TierConfig) => {
  ...
  const handleSave = useCallback((tier: BadgeTier) => {
    ...
    if (tier !== "Diamond" && rawMax !== "" && rawMax.includes(".")) {
      toast.error("Max spend must be a whole number (no decimals)");
      return;
    }
    if (tier !== "Diamond" && (maxVal === null || isNaN(maxVal) || maxVal < 0)) {
      toast.error("Max spend must be a valid number ≥ 0");
      return;
    }
    ...
    setTiers((prev) => {
      const idx     = prev.findIndex((t) => t.tier === tier);
      const current = prev[idx];

      if (tier !== "Diamond" && maxVal !== null && current.minSpend >= maxVal) {
        toast.error(
          `${tier}: Min ($${current.minSpend.toLocaleString()}) must be less than Max ($${maxVal.toLocaleString()})`
        );
        return prev;
      }

      const updated = prev.map((t, i) =>
        i === idx ? { ...t, maxSpend: maxVal, discount: discountVal } : t
      );
      ...
      if (maxVal !== null) {
        ...
        if (nextIdx < updated.length) {
          ...
          if (nextTier.maxSpend !== null && cascadedMin >= nextTier.maxSpend) {
            toast.error(
              `${nextTier.tier} min would become $${cascadedMin.toLocaleString()} but its max is $${nextTier.maxSpend.toLocaleString()}. Update ${nextTier.tier} max first.`
            );
            return prev;
          }
    ...
    // TODO: API call → PATCH /api/admin/discount-tiers
  }, [draft]);
  ...
  const filteredUsers = useMemo(() => {
    return MOCK_USERS.filter((u) => {
      ...
      const matchBadge = searchBadge === "all" || u.badge === searchBadge;
      return matchName && matchBadge;
    });
  }, [searchUsername, searchBadge]);

  const columns = useMemo<ColumnDef<UserBadgeRow, unknown>[]>(
    () => [
      {
        accessorKey: "userId",
        header: "USER ID",
        cell: ({ row }) => (
          <span className="font-mono text-c-slate-400 text-[12px]">
            {row.original.userId}
          </span>
        ),
      },
      {
        ...
        header: "USERNAME",
        cell: ({ row }) => (
          <span className="text-c-slate-200 font-medium text-[13px]">
      ...
      },
      {
        ...
        header: "TOTAL SPEND",
        cell: ({ row }) => (
          <span className="text-c-slate-300 font-semibold text-[13px]">
    ...
    <div className="space-y-6 w-full">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-c-slate-500 mt-0.5">
            Discounts are automatically applied to top-up based on all-time spend.
      ...
      <div className="bg-c-bg-700 border border-c-slate-800 rounded-xl p-5 shadow-sm space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <h2 className="text-sm font-semibold text-c-slate-200 tracking-wide">
            All Users
          </h2>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            ...
            <Button
              ...
              variant="ghost"
              size="sm"
              className="h-9 px-3 bg-c-rose-500/10 text-c-rose-500 hover:bg-c-rose-500/20 border border-c-rose-500/20 shadow-none"
            >
